...
SECURITY NOTE: WiFi credentials are now read from environment variables:
- ROBOT_WIFI_SSID: Your WiFi network name
- ROBOT_WIFI_PASSWORD: Your WiFi password  
- ROBOT_AGENT_IP: Agent IP address (default: 192.168.1.100)
- ROBOT_AGENT_PORT: Agent port (default: 8090)

Set these before running:
  export ROBOT_WIFI_SSID="your_ssid"
  export ROBOT_WIFI_PASSWORD="your_password"
  export ROBOT_AGENT_IP="192.168.68.105"
...
if __name__ == '__main__':
    ...
    # Get WiFi credentials from environment variables or .env file
    wifi_ssid = os.environ.get('ROBOT_WIFI_SSID', 'your_wifi_ssid_here')
    wifi_password = os.environ.get('ROBOT_WIFI_PASSWORD', 'your_wifi_password_here')
    
    # Get agent IP from environment or use default
    agent_ip = os.environ.get('ROBOT_AGENT_IP', '192.168.1.100')
    agent_port = int(os.environ.get('ROBOT_AGENT_PORT', '8090'))
    
    # Parse IP address
    ip_parts = agent_ip.split('.')
    if len(ip_parts) != 4:
        print("Invalid IP address format. Using default.")
        ip_parts = ['192', '168', '1', '100']
    
    robot.set_wifi_config(wifi_ssid, wifi_password)
    robot.set_udp_config([int(ip_parts[0]), int(ip_parts[1]), int(ip_parts[2]), int(ip_parts[3])], agent_port)
