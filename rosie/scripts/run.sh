...
for arg in "$@"; do
    ...
    case $arg in
        ...
        --help|-h)
            ...
            echo "      Test procedure:"
            echo "        1. Parses calendar_events.md for events today and in the upcoming week"
            echo "        2. Randomly selects an event and generates a question about it"
            echo "           Questions include: 'what is happening today?', 'what is on [day]?',"
            echo "           'when is [event name]?'"
            echo "        3. ROSIE's response is checked for event name, date, or time keywords"
