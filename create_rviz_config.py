def create_rviz_config(self):
    ...
    try:
        # Define the path for the RViz config
        rviz_config_dir = os.path.join(self.repo_root, 'config')
        rviz_config_path = os.path.join(rviz_config_dir, 'waypoint_nav.rviz')
        
        # Create the config directory if it doesn't exist
        if not os.path.exists(rviz_config_dir):
            os.makedirs(rviz_config_dir)
            self.get_logger().info(f'Created RViz config directory: {rviz_config_dir}')
        
        # Only create the config file if it doesn't exist
        if not os.path.exists(rviz_config_path):
            # Basic RViz config with visualization markers enabled
            rviz_config = """
Panels:
...
"""
            ...
            with open(rviz_config_path, 'w') as f:
                f.write(rviz_config)
            self.get_logger().info(f'Created RViz config file: {rviz_config_path}')
        else:
            self.get_logger().info(f'RViz config file already exists: {rviz_config_path}')
    except Exception as e:
        self.get_logger().error(f'Failed to create RViz config: {e}')
        import traceback
        traceback.print_exc()
